...
## 使用远程服务器作为开发环境
本地开发机的资源和性能总是有限的，这时候我们需要借助高性能的服务器来开发测试，在 `vscode` 使用 `Remote-SSH` 连接远程服务器与连接本地 WSL 一样简单，使用 `Ctrl+Shift+P` 命令后选择 `Connect to Host`，按照提示填写服务器的用户、IP、SSH 监听端口以及密码之后就可以如同在本地一样，在远程服务器上编写、调试代码。从此再也不用担心运行耗性能的程序会让本地机器死机了，而且服务器的环境也可以更贴近真实的生产环境，及早发现一些环境导致的问题。也可以多人共享一个统一的开发环境，甚至维护同一份代码（需谨慎）。
...
在 `vscode` 中只需要安装 `Dev Containers` 插件就可以快速启动一个容器来作为你的开发环境
...
要做到上述流程，`Dev Containers` 是通过 `.devcontainer` 文件夹下的 `devcontainer.json` 来自定义配置的，如容器名称、镜像名称等,除此之外还可以有 `Dockerfile`、`Docker Componse` 等配置。
...
在命令面板中使用 `Dev Containers: Attach to Running Container...`, 然后选择正在运行的容器即可，`Dev Containers` 插件会自动在运行容器中启动 `vscode server`, 然后驱动本地 `vscode` 进行远程连接, 如果没有多余的设置，默认不会打开一个容器内的文件夹，需要手动选择打开文件夹，然后就可以进行正常的调试了，这种一般用于调试正在运行的服务代码，比较适合调试新技术等第三方代码的场景。
...
有两种方式可以达到这个目的：
1. 使用 `Remote-SSH` 插件连接（官方推荐，本地不需要安装 Docker）:
...
2.  使用 Docker 客户端, 通过 `socket` 的方式进行连接(本地需要安装 Docker 和 `vscode` 插件)：
	...
	```
配置完成之后 `Dev Containers` 插件在新建开发容器的时候就会默认使用配置的远程 Docker 服务
...
4. 以上几个命令执行后， `vscode` 会先自动借助 `Remote-SSH` 插件连接上远程服务器，然后创建存储卷并挂载、运行容器，最后使用本地 `vscode` UI 连接上容器中的 `vscode server` 并打开 `workdir`。	
...
在使用这个插件之前，我们需要先安装好 `Remote-SSH` 插件，配置好 [kubeconfig](https://kubernetes.io/docs/tasks/access-application-cluster/configure-access-multiple-clusters/)。
